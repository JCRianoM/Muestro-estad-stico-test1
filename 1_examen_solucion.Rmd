...
est_total_simple<- function(N, n, df){
    ...
    df <- as.data.frame(df)
    table <- matrix(NA, nrow = 10, ncol = (dim(df)[2]))
    ...
    for (k in 1:dim(df)[2]) {
        ...
        col_noms <- c('Int. Inferior', 'Media muestra', 'Int. Superior', 
                      ...
                      'Int. Inferior total', 'Media Total', 
                      'Int. Superior Total', 'Varianza Total')
        table[, k] <- c(Inf_int_s, med_s, Sup_int_s, sd_s, var_var,
...
```{r cosa end, echo=FALSE, message=FALSE, results='asis'}
xp <- est_total_simple(4898, 254, wine)
```

```{r estimadores finales tttt sys1, echo=FALSE, message=FALSE, results='asis'}
panderOptions('missing', '---')
pandoc.table(xp,
             caption = 
             'Tabla estimadores de la muestra y totales 
